...
class Agent:
    ...
    def execute_task(self, t):
        common_reward = {}
        if self.task:
            reward, complete, success = self.task.execute(t,self.task_start,self.skills[self.task])
            if complete:
    ...
    def get_average_skill_for_property(self):
        return {prop:np.average([self.skills[task] for task in self.property_task_mapping[prop]]) for prop in self.property_task_mapping.keys()}
